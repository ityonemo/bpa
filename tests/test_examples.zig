...
pub fn addTests(
    ...
    ctx.ok(&.{ "check", "examples/euclid.bpa" }, "OK: 237 declarations, 59 theorems proven\n");

    ctx.ok(&.{ "check", "examples/euclid-compute.bpa" }, "OK: 260 declarations, 61 theorems proven\n");

    // √2 is irrational (stated over ℕ), proven — the headline result.
    ctx.ok(&.{ "check", "examples/sqrt2.bpa" }, "OK: 292 declarations, 62 theorems proven\n");
